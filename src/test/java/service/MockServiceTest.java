...
public class MockServiceTest {
    ...
    public void 어떠한챔피언이름을입력해도무조건베인리턴테스트() {
        Champion champion = mock(Champion.class);
        champion.setName("케이톨린");
        when(champion.getName()).thenReturn("베인");
        assertThat(champion.getName(), is("베인"));
    }

    // 1-1. 어떤 포지션을 set해도 미드를 리턴하도록 테스트 하세요
    @Test
    public void 어떤포지션을셋해도미드리턴테스트() {
        Champion champion = mock(Champion.class);
        champion.setPosition("미드드");
    ...
    public void shouldDothrowExceptionChampionSetNameYasuo() {
        Champion champion = mock(Champion.class);
